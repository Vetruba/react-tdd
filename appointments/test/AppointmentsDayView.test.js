...
describe('Appointment', () => {
  ...
  it('renders the customer last name', () => {
    customer = { firstName: 'Ashley', lastName: 'Babet' };
    render(<Appointment customer={customer} />);

    expect(container.textContent).toMatch('Babet');
  })

  it('renders another customer last name', () => {
    customer = { firstName: 'Jordan', lastName: 'Peterson' };
    document.body.appendChild(container);
    render(<Appointment customer={customer} />);
    expect(container.textContent).toMatch('Peterson');
  });

  it('renders the customer phone number', () => {
    customer = { firstName: 'Ashley', lastName: 'Babet', phoneNumber: '1234567890' };
    render(<Appointment customer={customer} />);

    expect(container.textContent).toMatch('1234567890');
  })

  it('renders the stylist name', () => {
    customer = { firstName: 'Ashley', lastName: 'Babet', phoneNumber: '1234567890' };
    render(<Appointment customer={customer} stylist='Linda' />);

    expect(container.textContent).toMatch('Linda');
  })
